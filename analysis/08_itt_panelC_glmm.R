# This script performs the ITT Panel C GLMM analysis.
# In this sensitivity-analysis dataset, all assigned-but-unanswered items are
# coded as incorrect in both study arms.
...
data1 <- read_excel("data/LungDiag-GPT4o_itt_panelC_na_as_incorrect.xlsx")
data2 <- read_excel("data/control_itt_panelC_na_as_incorrect.xlsx")
...
doc <- body_add_par(
  doc,
  value = "Panel C: all assigned-but-unanswered items coded as incorrect in both arms",
  style = "Normal"
)
...
print(doc, target = "itt_panelC_glmm_results.docx")

cat("\nGLMM analysis completed. Results were saved to itt_panelC_glmm_results.docx\n")