# This script performs the ITT Panel B GLMM analysis.
# In this sensitivity-analysis dataset, all assigned-but-unanswered items are
# coded as correct in both study arms.
...
data1 <- read_excel("data/ai_itt_panelB_na_as_correct.xlsx")
...
doc <- body_add_par(
  doc,
  value = "Panel B: all assigned-but-unanswered items coded as correct in both arms",
  style = "Normal"
)
...
print(doc, target = "itt_panelB_glmm_results.docx")

cat("\nGLMM analysis completed. Results were saved to itt_panelB_glmm_results.docx\n")